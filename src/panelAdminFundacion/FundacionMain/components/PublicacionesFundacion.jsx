...
export const PublicacionesFundacion = ({ soloConvocatorias = false }) => {
  ...
  const handleSearch = async (limite) => {
    const { convocatorias } = await apiConvocatorias((limite - 1) * maxResultsPerPage, limite * maxResultsPerPage);
    setConvocatorias(convocatorias);
  };
  ...
  return (
    <>
      <h5>Tus convocatorias abiertas:</h5>
      {soloConvocatorias===false ? [] : (
        <Link to="/historial-convocatorias" className="btn btn-danger btn-block mb-3 mt-2">
